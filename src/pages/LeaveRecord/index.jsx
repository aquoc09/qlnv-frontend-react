...
const LeaveBalanceList = () => {
    ...
    // 1. LẤY DATA THẬT TỪ BE (Bóc tách mảng trực tiếp như ảnh Console anh chụp)
    const fetchData = async () => {
        ...
        try {
            ...
            // FIX: BE trả về Array trực tiếp (nhìn cột Console anh chụp là thấy Array(14) và Array(5))
            ...
            console.log("Data Quỹ phép đã bóc tách:", balanceData);
        } catch (error) {
    ...
    // HÀM TÌM TÊN NHÂN VIÊN: Đối chiếu ID quỹ phép với danh sách nhân viên
    ...
    // 2. XỬ LÝ CẬP NHẬT (SỬA SỐ NGÀY PHÉP TRỰC TIẾP TRÊN MYSQL)
    const handleUpdate = async (e) => {
        e.preventDefault();
        const formData = new FormData(e.target);
        
        // Ép kiểu số để BE Java nhận được
        const data = {
        ...
        try {
            ...
            alert("Lỗi: " + (error.response?.data?.message || "Không thể cập nhật (999)"));
        }
    };
                        ...
                        {balances.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan="5" style={{ textAlign: 'center', padding: '40px' }}>
                                    <b>Bảng trống. Kiểm tra lại MySQL hoặc quyền truy cập.</b>
                                </td>
                            </tr>
                        )}
                    </tbody>
